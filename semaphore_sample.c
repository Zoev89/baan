...
/* Warning: this example does not check the status return of any Pthread or
 *          semaphore library function, and therefore merits a lowered grade.
 *
...
/* On Solaris, compile with   cc -mt semaphore_sample.c -lpthread -lrt
 *             or             gcc semaphore_sample.c -lpthread -lrt
 * run with  a.out
 *           a.out 1
 *           a.out 2
 *           a.out 3
...
/* function prototype for Pthread creation */
...
int main(int argc, char * argv[])
{
  ...
  pthread_t tid[2];     /* thread ID, set by pthread_create() */
  int ret[2] = {2, 4};
        /* passed to thread as argument by pthread_create()
         * received from thread as return value by pthread_join()
         */
  ...
  pthread_create(&tid[0], NULL, thread_func, (void *)ret[0]);
  pthread_create(&tid[1], NULL, thread_func, (void *)ret[1]);

  printf("main: process id %d, thread id = %d\n", getpid(), pthread_self());
  printf("main: tid[0] = %d, tid[1] = %d\n", tid[0], tid[1]);
  printf("main: ret[0] = %d, ret[1] = %d\n", ret[0], ret[1]);

  sem_getvalue(&count_sem, &v);
  ...
  if (quit == 3 && ret[1] < 0)
    printf("main: thread %d return = %s\n", tid[1], strerror(-ret[1]));
...
void * thread_func(void * arg)
{
  ...
  printf("thread: process id %d, thread id = %d, arg = %d, count = %d, s = %d\n",
      getpid(), pthread_self(), (int)arg, count, s);
  ...
  printf("thread: process id %d, thread id = %d, arg = %d, count = %d, s = %d\n",
      getpid(), pthread_self(), (int)arg, t, s);

  if (quit == 4) pthread_exit((void *)(-pthread_self()));
  if (quit == 5) exit(0);
